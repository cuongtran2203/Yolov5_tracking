...
def get_area_detect(img, points):
    ...
    return dts
def estimate_velocity(len_contours=None,A=[None,None],B=[None,None],FPS=None) :
    ...
    speed=distance_in_meter_zone
    return speed*3.6*FPS
...
class Tracking():
    ...
    def infer(self,img:np.ndarray):
        ...
        # print(fps)
        speed_list.clear()
        return online_im,list_count,cs,current_dict_info,bbox,fps
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if p1 is not None and p2 is not None and p3 is not None and p4 is not None :
            ...
            #Tracking
            img_s,list_count,cs,dict_info,bbox,fps=track.infer(img_croped)
            for box in bbox :
            ...
            if len(dict_info)>0 and len(prev_dict_id)>0:
                for key in dict_info.keys():
                    if key in prev_dict_id.keys():
                        ...
                        B=dict_info[key][0]
                        deta_t=abs(dict_info[key][1]-prev_dict_id[key][1])
                         
                        speed=estimate_velocity(Max_contours,A,B,fps)
